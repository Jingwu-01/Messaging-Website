    ...
    constructor(response: PostResponse) {
        ...
        if (response.doc.parent === undefined || response.doc.parent === "") {
            ...
            if (parentPathArr.length !== 6) {
                throw new Error("AdapterPost constructor: parentPathArr is not of the correct length");
            }
            let parentName = parentPathArr[5];
            if (postPathArr[1] !== parentPathArr[1]) {
                throw new Error("AdapterPost constructor: workspace name of parent and child are not equal");
            }
            if (postPathArr[3] !== parentPathArr[3]) {
                throw new Error("AdapterPost constructor: channel name of parent and child are not equal");
            }
            if (parentName === undefined) {
                throw new Error("AdapterPost constructor: internal server error (last element of parentPathArr is undefined)");
            }
