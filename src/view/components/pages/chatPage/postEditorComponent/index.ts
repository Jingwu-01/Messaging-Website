...
export class PostEditor extends HTMLElement {
    ...
    connectedCallback() {
        ...
        this.postForm.addEventListener("submit", this.submitPost.bind(this), options);
    }
    ...
    submitPost(event: SubmitEvent) {
        ...
        slog.info("submitPost", ["createPostEvent.detail", `${JSON.stringify(createPostEvent.detail)}`]);
    ...
    printParentEl() {
        slog.info("printParentEl", ["postEditor parent", `${JSON.stringify(this.parentNode)}`], ["postEditor", `${JSON.stringify(this)}`]);
    }
