...
function stringAdditionHelper(
  keyName: string,
  dataPair: [string, boolean],
  last: boolean,
) {
  if (dataPair[1]) {
    if (last) {
...
function getReactionObject(
  smile: [string, boolean],
  frown: [string, boolean],
  like: [string, boolean],
  celebrate: [string, boolean],
  additionalReaction: [string, string, boolean],
) {
  if (!additionalReaction[2]) {
    ...
    return `{
            ...
            ${stringAdditionHelper(
              additionalReaction[0],
              [additionalReaction[1], additionalReaction[2]],
              true,
            )}
        }`;
  }
...
function getDocumentBodies(
  databasePath: string,
  username: string,
): Map<string, string> {
  return new Map<string, string>([
    [
      "existingworkspace_onechannel",
      getDocumentResult(
        "existingworkspace_onechannel",
        "{}",
        databasePath,
    ...
    [
      "existing_onechannel_onepost",
      getDocumentResult(
        "existing_onechannel_multposts",
        "{}",
        databasePath,
        username,
        1701873177565,
        1701873257345,
        "/existingworkspace_onechannel/channels",
      ),
    ],
    [
      "existing_post1",
      getDocumentResult(
        "multposts_post1",
        getPostBody("test message 1", ["", false], ["", false], ["", false]),
        databasePath,
        username,
        1701876075240,
        1701876083511,
        "existingworkspace_onechannel/channels/existing_onechannel_onepost/posts",
      ),
    ],
    ...
    [
      ...
      getDocumentResult(
        ...
        1701873686563,
        1701873691816,
        "/workspace_onechannel/channels/onechannel_multposts/posts",
      ),
    ],
    [
      "multposts_post2",
      getDocumentResult(
        "multposts_post2",
        getPostBody(
          "another test message for post 2. contains reactions",
          [
            `"${databasePath}/channels/onechannel_multposts/multposts_post1"`,
            true,
          ],
          [
            getReactionObject(
    ...
    [
      ...
      getDocumentResult(
        "multposts_post3",
        getPostBody(
          "some post content for post 3. contains extensions :smile::like::celebrate:**bold***italic*[text](https://www.google.com])",
          ["", false],
          ["", false],
          [getExtensionObject([`[${username}]`, true]), true],
        ),
        databasePath,
        username,
        1701873686563,
    ...
    [
      ...
      getDocumentResult(
        ...
        1701873177565,
        1701873257345,
        "/workspace_multchannels/channels",
      ),
    ],
    [
      "multchannels_onepost",
      getDocumentResult(
        "multchannels_noposts",
        "{'channel data': 'channel value'}",
        databasePath,
    ...
    [
      "onepost_ <script>alert('this is an attack!')</script>",
      getDocumentResult(
        "onepost_ <script>alert('this is an attack!')</script>",
        getPostBody(
        ...
        1701874873310,
        1701874878667,
        "/workspace_multchannels/channels/multchannels_onepost/posts",
      ),
    ],
  ]);
}

export const fetchFunc = jest.fn(
  (input: RequestInfo | URL, init?: RequestInit): Promise<Response> => {
    ...
    switch (input) {
      ...
      case `${dbUrl}/existingworkspace_onechannel`:
        switch (method) {
          case "GET":
            ...
            slog.info("called mock fetch existingworkspace_onechannel", [
              "existingworkspace_onechannel body",
              documentBodies.get("existingworkspace_onechannel"),
            ]);
            break;
          case "PUT":
            console.log(
              "should never be overwriting an existing channel. this is an error.",
            );
            break;
          case "DELETE":
            status = 204;
      ...
      case `${dbUrl}/existingworkspace_onechannel?timestamp=0`:
        switch (method) {
          case "PUT":
            ...
            statusText = "Bad Request";
            body =
              '"unable to create/replace doc: timestamp 0 does not match precondition timestamp of 1701876029328"';
            break;
        }
        break;
      ...
      case `${dbUrl}/existingworkspace_onechannel/channels/existing_onechannel_onepost`:
        switch (method) {
          ...
          case "PUT":
            console.log(
              "this operation will overwrite an existing channel and is an error.",
            );
            break;
          case "DELETE":
            status = 204;
      ...
      case `${dbUrl}/existingworkspace_onechannel/channels/existing_onechannel_onepost?timestamp=0`:
        switch (method) {
          case "PUT":
            ...
            statusText = "Bad Request";
            body =
              '"unable to create/replace doc: timestamp 0 does not match precondition timestamp of 1701873257345"';
            break;
        }
        break;
      ...
      default:
        ...
        console.log(
          "error: sending requests to server outside of supported operations",
        );
        break;
    }
    ...
    slog.info("mockFetch: resPromise passed", [
      "resPromise.status",
      resPromise,
    ]);
